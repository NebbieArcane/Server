...
{
   char buf[500];
   snprintf(buf,499,"$c0115Non ti sembra che %d hp siano davvero un po' troppi?$c0007\n\r",
	   hp);
   send_to_char(buf,ch);
   SET_BIT(ch->specials.act,PLR_FREEZE);
   buglog(LOG_PLAYERS,"%s congelato per HP",
	  GET_NAME(ch));
...
void load_one_room( FILE *fl, struct room_data *rp )
{
  ...
  rp->name = fread_string(fl);
   if (IsTest()) 
   {
      mudlog(LOG_CHECK,"LR: vnum %d name %s",rp->number,rp->name);
   }
...
/*read riga2 Nome della zona~ */
	 ...
	 bc += 10;
      ...
      buf[20]='\0';
      mudlog(LOG_CHECK,"#%d(%d): %s %d-%d %dm %d",
	     zon,
	     zone_table[zon].num,
	     buf,
...
void store_to_char(struct char_file_u *st, struct char_data *ch)
{
   ...
   mudlog(LOG_SAVE,"<-Mana/Hits prima di reload: %d/%d",GET_MAX_MANA(ch),GET_MAX_HIT(ch));
   ch->points = st->points;
   mudlog(LOG_SAVE,"<-Mana/Hits dopo reload    : %d/%d",GET_MAX_MANA(ch),GET_MAX_HIT(ch));
   mudlog(LOG_SAVE,"<-MMana/MHits from points: %d/%d",ch->points.max_mana,ch->points.max_hit);
   mudlog(LOG_SAVE,"<- Mana/ Hits from points: %d/%d",ch->points.mana,ch->points.hit);
...
/* Specials */   
   ...
   mudlog(LOG_SAVE,"<-Mana/Hits dopo affecttot : %d/%d",GET_MAX_MANA(ch),GET_MAX_HIT(ch));
...
void free_char(struct char_data *ch)
{
   ...
   {
     if (auction->seller == ch) auction->seller = NULL;
...
void reset_char(struct char_data *ch)
{
  struct affected_type *af;
   ...
   if (GET_HIT(ch) <= 0)
     GET_HIT(ch) = 1; /*** SALVO il regen parte prima di uscire ***/
   if (GET_MOVE(ch) <= 0)
     GET_MOVE(ch) = 1;
   if (GET_MANA(ch) <= 0)
     GET_MANA(ch) = 1;
...
/* Imposta i livelli di default obbligatori per gli dei */
  ...
  if( !strcmp(GET_NAME(ch),"Xanathon")) 
  {
    GET_LEVEL(ch,0) = 60;
  }
  if( !strcmp(GET_NAME(ch),"Urhar")) 
  {
    GET_LEVEL(ch,0) = 59;
  }
 ...
 * */
   ...
   {
      if (IS_PC(ch) && !IS_IMMORTALE(ch) && !IS_SET(ch->player.user_flags,RACE_WAR))
      {
	 send_to_char("$c0115      ATTENTO! ADESSO SEI PKILL                $c0007\n\r",ch);
   ...
   if (ratio > 1.0)      
   {
      buglog(LOG_SYSERR,"%s ha %d hp (stimati: %f) ratio %f",
	     GET_NAME(ch),hit_limit(ch),absmaxhp,ratio);
   }
   ...
   if( ( dir = opendir( PLAYERS_DIR ) ) != NULL )
   {
	 ...
	 if( strstr(ent->d_name,".dat")) 
	 {
	    ...
	    if (( pFile = fopen( szFileName, "r+" ) ) != NULL )
	    {
		  ...
		  {
		     buglog(LOG_PLAYERS,"%s (%s) ha piu' di 500 hp (%d)",
			    grunt.dummy.name,
			    classes,
		  ...
		  /*gli immortali non si cancellano*/
		  if( max < IMMORTALE )
		  {
		     j = 1;
		     if( max > 3 )
		     ...
		     /* Purge rent files! after inactivity of RENT_INACTIVE months */
#ifndef NOREGISTER
		     if( !grunt.AXE && age >
			(long) RENT_INACTIVE * ( SECS_PER_REAL_DAY * 30) )
		     {
			mudlog( LOG_PLAYERS, 
			       "Purging rent file for %s, level %d, inactive for %d months.",
			       grunt.dummy.name, max, RENT_INACTIVE );
			sprintf( buf, "rm -f %s/%s", RENT_DIR, lower( grunt.dummy.name ) );
			system( buf );
		     }
#endif
#endif
/* BUG BUG BUG */                                                          
		     ...
		     if( !grunt.AXE && age >
			(long) j * ( SECS_PER_REAL_DAY * 7 ) &&
			!IS_SET( grunt.dummy.user_flags, NO_DELETE ) )
		     {
			num_deleted++;
			regdelete(grunt.dummy.name);
			grunt.AXE = TRUE;
                        mudlog( LOG_PLAYERS, "%s marked for deletion after %d weeks of inactivity.",                                                  
			       grunt.dummy.name, j );
		     }  
			   ...
			   (long) ( j ) * ( SECS_PER_REAL_DAY * 7 ) )
			{
			   ...
			   grunt.AXE = TRUE;
			   mudlog( LOG_PLAYERS, 
                                  "%s marked for deletion after %d weeks of inactivity (NO_DELETE).", 
				  grunt.dummy.name,j );
			}
		     }
		     /* Avviso di cancellazione imminente*/
		     if( !grunt.AXE && age >
			(long) (j-1) * ( SECS_PER_REAL_DAY * 7 ) &&
			!IS_SET( grunt.dummy.user_flags, NO_DELETE ) )
		     { 
			num_warned++;
			life= (long) (j*7) - (age / SECS_PER_REAL_DAY);
			if (life < 2)
			  mudlog( LOG_PLAYERS, "%s to be deleted in %d day",
		  ...
		  else if( max >= IMMORTALE )
		  {
                     ...
                     {
                        mudlog( LOG_PLAYERS,
                               "Gli immortali devono essere asceti: %s.",
                               grunt.dummy.name);
                        sprintf( buf, "/bin/rm -f %s/%s", RENT_DIR, lower( grunt.dummy.name ) );
                        system( buf );
                     }                                                     
		     ...
		     {
			num_deleted++;
			grunt.AXE = TRUE; mudlog( LOG_PLAYERS, "%s marked for deletion (TOOHIGHLEVEL).",
			       grunt.dummy.name );
		     }/* Retrocessione per gli dei inattivi */
                      /* Se il dio inattivo e' un IMMO del 52
                         retrocede al livello 50  GAIA 2001 */
		     else if( 
			...
			num_demoted++;
			mudlog( LOG_PLAYERS, 
			       "%s demoted from %d to %d and cleared due to inactivity.",
			       grunt.dummy.name, max, max - 1 );
			sprintf( buf, "/bin/rm -f %s/%s", RENT_DIR, lower( grunt.dummy.name ) );
			system( buf );
			grunt.dummy.last_logon = timeH; 
			/* so it doesnt happen twice */
                        if(max == IMMORTALE) 
		        {
                        max--;
		         }
			...
			for( j = 0; j < MAX_CLASS; j++ )
			grunt.dummy.level[j] = MIN(grunt.dummy.level[j],max);
		  ...
		  mudlog( LOG_ERROR, "Error %s reading file %s.", 
	       ...
	       {
#ifndef NOREGISTER	 
		  sprintf( buf, "/bin/rm -f \"%s/%s.dat\"", PLAYERS_DIR, lower( grunt.dummy.name ) );
		  system( buf );
 sprintf( buf, "/bin/rm -f \"%s/%s.dead\"", PLAYERS_DIR, lower(
grunt.dummy.name ) );
                  system( buf );
		  sprintf( buf, "/bin/rm -f \"%s/%s\"", RENT_DIR, lower( grunt.dummy.name ) );
		  system( buf );
		  sprintf( buf,"/bin/rm -f \"%s/%s.aux\"", RENT_DIR, lower( grunt.dummy.name ));
		  system( buf );
                  mudlog(LOG_CHECK,"%s cancellato",grunt.dummy.name);      
#else
...
void Start_Auction() 
{
   ...
   if (auction == NULL)
   {
      mudlog (LOG_ERROR,"malloc'ing AUCTION_DATA didn't give %d bytes",sizeof(AUCTION_DATA));
      exit (1);
   }
