          ...
          if (trueGain <= xpcum || GET_EXP(ch)>=PRINCEEXP) {
              ...
              if (max>0) {
                ...
                act("Non puoi apprendere cosi tanto!!\r\n",FALSE,mob,0,ch,TO_VICT);
				  	    act("$N declama le rune, ma non accade nulla!\r\n",FALSE,mob,0,ch,TO_NOTVICT);
				  	    mudlog(LOG_PLAYERS, "esecuzione rune --> %s non può convertire rune ulteriormente (overflow principi)",GET_NAME(ch));
                return true;
              }
            return true;
            }
            ...
            max=(xpcum-GET_EXP(ch))*k/(GetMaxLevel(ch)*10000);
              ...
              act("Non puoi apprendere cosi tanto!!\r\n",FALSE,mob,0,ch,TO_VICT);
				  	  act("$N declama le rune, ma non accade nulla!\r\n",FALSE,mob,0,ch,TO_NOTVICT);
				  	  mudlog(LOG_PLAYERS, "esecuzione rune --> %s non può convertire rune ulteriormente (maxxaggio classe)",GET_NAME(ch));
              return true;
				  	}
  				}	
  			} else {
  ...
  if ((GET_POS(mob)==POSITION_FIGHTING) && (number(0,9)<6)) {
    ...
    if(mob->specials.fighting)
    {
      ...
      if(IS_NPC(mobtmp))
      {
        if(mobtmp->nr==nummob)
        {
          // Ok, è una delle bambole che sta combattendo
    ...
    // Se il MOB o il PG é immune a drain non perdo nemmeno il tempo di tentare
    // il che è bastardo perché non vedi il messaggio quindi non sa che drena :-)
    if(!(IS_SET((mob)->M_immune, IMM_DRAIN)))
      //IS_IMMUNE(mob,IMM_DRAIN) ARGH!!! IS_IMMUNE qui non è nemmeno definita!
    {
