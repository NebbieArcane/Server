...
int special(struct char_data *ch, int dire, char *arg);
...
int ValidMove( struct char_data *ch, int cmd) 
{
   ...
   if (affected_by_spell(ch, SPELL_WEB)) 
   {
	 ...
	 WAIT_STATE(ch, PULSE_VIOLENCE); // web
         GET_MOVE(ch) -=50; alter_move(ch,0);
   ...
   else 
   {
     ...
     /* Added for Pkillers, they cannot enter peaceful rooms
        Gaia 2001  */
      if (IS_AFFECTED2((IS_POLY(ch)) ? ch->desc->original : ch, AFF2_PKILLER) && // SALVO controllo se polato
            IS_SET( rp->room_flags , PEACEFUL))  {
            send_to_char( "Una forza arcana ti impedisce di andare in quella direzione. \n\r", ch); 
              return(FALSE);
            } 
...
int RawMove( struct char_data *ch, int dir, int bCheckSpecial )
{
   ...
   if (!IS_IMMORTAL(ch) || MOUNTED(ch)) 
   {
	 ...
	 if (MOUNTED(ch)) 
	 {
            GET_MOVE(MOUNTED(ch)) -=need_movement; alter_move(ch,0);
	 } 
	 else 
	 {
            GET_MOVE(ch) -=need_movement; alter_move(ch,0);
	 }
      }
   }
   ...
   if (IS_NPC(ch)) 
   {
	 ...
	 if (IS_SET(ch->specials.act, ACT_HUNTING) && ch->desc)
	   WAIT_STATE(ch, PULSE_VIOLENCE); // NPC track
      }
   } 
   else 
   {
      ...
      if ((ch != tmp_ch) && (AWAKE(tmp_ch)) && (CAN_SEE(tmp_ch, ch))) 
      {
        if (!IS_AFFECTED(ch, AFF_SILENCE) || number(0,2)) 
        {
          ...
          else 
          {
            ...
            {
              if (MOUNTED(ch)) 
                snprintf( tmp,DMMAX,"%s va %s, cavalcando %s\r\n",
        ...
        (CAN_SEE(tmp_ch,ch)) &&        (AWAKE(tmp_ch))) 
    {
      if (tmp_ch != ch && (!IS_AFFECTED(ch, AFF_SILENCE) || number(0,2))) 
      {
        if (dir < 6) 
        {
          ...
          {
            if (MOUNTED(ch)) 
              snprintf(tmp,DMMAX, "%s arriva %s, cavalcando %s.\r\n", 
          ...
          {
            if (IS_AFFECTED(ch,AFF_FLYING)) 
              snprintf(tmp,DMMAX, "%s arrivano volando %s [%d].\r\n", 
                      PERS(ch, tmp_ch),dirsFrom[rev_dir[dir]],total);
            else
        ...
        else   /* dir >= 6 ossia non viene da una direzione particolare */
        {
          ...
          {
            if (IS_AFFECTED(ch,AFF_FLYING)) 
              snprintf( tmp,DMMAX, "Arrivano %s volando. [%d]\r\n", 
    ...
    else if( exitp->open_cmd != -1 && cmd == exitp->open_cmd ) 
    {
      if( !IS_SET( exitp->exit_info, EX_CLOSED ) ) 
      {
        if( cmd != CMD_DIG && cmd != CMD_SCYTHE )
        {
          ...
          if( IS_SET( exitp->exit_info, EX_SECRET ) ) 
          {
            sprintf(buf,"$n chiude un passaggio segreto %s",dirsTo[door]);
      ...
      else 
      {
            ...
            ( cmd != CMD_DIG && cmd != CMD_SCYTHE ) )
        {
          ...
          {
            sprintf( buf,"$n %s %s $F ed apre un passaggio %s", cmdnameT,
                     IS_SET( exitp->exit_info, EX_MALE ) ? "il" : "la",
                     dirsTo[door]);
            act(buf, 0, ch, 0, exitp->keyword, TO_ROOM);
          }
...
void do_run(struct char_data *ch, char *argument, int cmd)
{
  ...
  {
    send_to_char( "Una forza arcana ti impedisce di correre da quella parte" , ch );
    return;
  } 
