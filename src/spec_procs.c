   ...
   switch (cmd) {
    ...
    case CMD_DISARM:
	 ...
	 if (hitsleft < GET_HIT(ch) && GetMaxLevel(ch) <= MAX_MORT) {
	    ...
	    GET_HIT(ch) = hitsleft; alter_hit(ch,0);
	 } else {
...
int regenerator( struct char_data *ch, int cmd, char *arg, struct char_data *mob, int type)
{
  if( type == EVENT_TICK )
  {
    ...
    {
      GET_HIT(ch) +=9;
      GET_HIT(ch) = MIN(GET_HIT(ch), GET_MAX_HIT(ch)); alter_hit(ch,0);
      ...
      if (GET_MANA(zmaster) < ZM_MANA) {
        ...
        if (!check_soundproof(ch)) {
          act("$n says 'Wonderful, another loyal follower!' and grins maniacly.",
            FALSE, zmaster, 0,0, TO_ROOM);
          GET_MANA(zmaster) -= ZM_MANA; alter_mana(zmaster,0);
  ...
  if( AWAKE(ch) )
  {
    if( ( targ = FindAnAttacker(ch) ) != NULL )
    {
      ...
      if (!saves_spell(targ, SAVING_PARA)) 
      {
        ...
        /* kill target: */
        GET_HIT(targ) = 0; alter_hit(targ,0);
...
void String_mob(struct char_data *ch, struct char_data *vict, char *stringa, int campo)
{
  ...
  if (*ch->desc->str) { free(*ch->desc->str); }
...
//  send_to_all("minor.\n\r");
  ...
  switch(i) 
      ...
      case 8:
        ...
        GET_MOVE(vict) -=10; alter_move(vict,0);
        break;
      case 9:
      ...
      case 10:
        ...
        GET_MANA(ch) +=100; alter_mana(ch,0);
        break;
      default:
...
//  send_to_all("major.\n\r");
  ...
  switch(i) 
      ...
      case 1:
        ...
        amnt=MAX(amnt,0);
        GET_HIT(vict)-=amnt; alter_hit(vict,0);
        update_pos(vict);
        break;
      case 2:
        ...
        GET_MOVE(vict)=0; alter_move(vict,0);
        break;
      case 3:
  ...
  if (number(1,10)==1) {
    ...
    if ((mage) && (!IS_IMMORTAL(mage)) && (!IS_NPC(mage))) {
           ...
           if (! (HasClass(mage, CLASS_MAGIC_USER) || HasClass(mage,CLASS_SORCERER)) ) {
             ...
             send_to_char("\n\r",mage);
             if (rakda->equipped_by) { rakda = unequip_char(rakda->equipped_by, rakda->eq_pos); }
             else { obj_from_char(rakda); }
...
#define MIN_WEARING 10
{
   ...
   act("$n si contorce come in agonia, mentre un vento impetuoso si alza dal nulla.", FALSE,
       ch, 0, god, TO_ROOM);
   GET_HIT(ch)=MIN(GET_HIT(ch),10); alter_hit(ch,0);
   for(xp=TYPE_GENERIC_FIRST;xp<=TYPE_GENERIC_LAST;xp++)
   {
#if NOSCRAP
   DamageStuff(ch,xp,200,5);
