#!/bin/bash
# Installa ODB 2.5 + libodb via build2/bpkg (bindist ubuntu24.04, gcc-13).
...
install_odb_packages() {
	...
	if [ ! -d "${ODB_BUILD}/odb-gcc-13" ]; then
		bpkg create -d "${ODB_BUILD}/odb-gcc-13" cc \
			config.cxx=g++-13 \
	...
	if ! command -v odb >/dev/null 2>&1; then
		run_retry bash -c "printf 'y\n' | bpkg build --trust-yes -d '${ODB_BUILD}/odb-gcc-13' 'odb@${BPKG_STABLE_REPO}'"
		bpkg install -d "${ODB_BUILD}/odb-gcc-13" odb
	fi

	if [ ! -d "${ODB_BUILD}/libodb-gcc-13" ]; then
		bpkg create -d "${ODB_BUILD}/libodb-gcc-13" cc \
			config.cxx=g++-13 \
			config.cc.coptions=-O2 \
			config.install.root=/usr/local
	fi
	bpkg add --trust-yes -d "${ODB_BUILD}/libodb-gcc-13" "${BPKG_STABLE_REPO}" 2>/dev/null || true
	run_retry bpkg rep-fetch -d "${ODB_BUILD}/libodb-gcc-13"
	run_retry bpkg fetch -d "${ODB_BUILD}/libodb-gcc-13"
	run_retry bash -c "printf 'y\n' | bpkg build --trust-yes -d '${ODB_BUILD}/libodb-gcc-13' libodb"
	run_retry bash -c "printf 'y\n' | bpkg build --trust-yes -d '${ODB_BUILD}/libodb-gcc-13' 'libodb-sqlite ?sys:libsqlite3'"
	run_retry bash -c "printf 'y\n' | bpkg build --trust-yes -d '${ODB_BUILD}/libodb-gcc-13' 'libodb-mysql ?sys:libmysqlclient'"
	run_retry bash -c "printf 'y\n' | bpkg build --trust-yes -d '${ODB_BUILD}/libodb-gcc-13' libodb-boost"
	bpkg install -d "${ODB_BUILD}/libodb-gcc-13" --all --recursive
