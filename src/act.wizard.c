...
/*   int player_i, pos;*/
   ...
   if( (cmdnum==CMD_SAVE) || load_char( name, &tmp_store ) )
   {
      if (cmdnum==CMD_SAVE)
      {
...
"$c0011    Remember, be $c0015careful$c0011 how you use this command!\n\r",ch);
  ...
  {
    GET_HIT(mob) = parm; alter_hit(mob,0);
  }
  ...
  else if (!strcmp(field, "prac")) 
  {
   ...
   else if (!strcmp(field, "mana")) 
   {
	 ...
	 send_to_char(buf,ch);
      ...
      {
         GET_MANA(mob) =parm; alter_mana(mob,0);
      }
   }
  ...
  else if( !strcmp(field, "move") )
  {
    ...
    {
      send_to_char( "OK.\n\r", ch );
      GET_MOVE(mob) =parm; alter_move(mob,0);
    }
  }
