...
/* fine modifica */
	 ...
	 GET_GOLD(victim)=0;
	 GET_EXP(victim)=0;
	 GET_HIT(victim)=1; alter_hit(victim,0);
                        ...
                        struct char_data *victim, struct obj_data *obj)
{
  ...
  {
	GET_HIT(victim) = hit_limit(victim); alter_hit(victim,0);
  }
  else
  {
                ...
                struct char_data *victim, struct obj_data *obj)
{
   ...
   if (level > 0)
   {
	  ...
	  {
		GET_MANA(ch) = GET_MAX_MANA(ch); alter_mana(ch,0);
	  }
	  else
	  {
		GET_MANA(ch) += dam; alter_mana(ch,0);
	  }
   }
   else 
   {
      ...
      GET_HIT(ch)=GET_MANA(ch); alter_hit(ch,0);
      GET_MANA(ch)=0; alter_mana(ch,0);
                       ...
                       struct char_data *victim, struct obj_data *obj)
{
  ...
  {
     GET_MOVE(victim) = move_limit(victim); alter_move(victim,0);
  }
  else
  {
     GET_MOVE(victim) += dam; alter_move(victim,0);
  }
                     ...
                     struct char_data *mob, struct obj_data *obj)
{
   ...
   GET_MANA(mob) = (GET_MANA(ch)); alter_mana(mob,0);
   ...
   struct char_data *victim, struct obj_data *obj)
{
  ...
  for (tch=real_roomp(ch->in_room)->people; tch; tch=tch->next_in_room) {
     if ((in_group_strict(tch, ch)) && (GET_POS(ch) > POSITION_SLEEPING)) {
	  ...
	  GET_HIT(tch) +=1; alter_hit(tch,0);
        }
        GET_MOVE(tch) = GET_MAX_MOVE(tch); alter_move(tch,0);
     }
  }
}
