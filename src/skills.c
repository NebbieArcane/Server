...
void slam_into_wall( struct char_data *ch, struct room_direction_data *exitp)
{
  ...
  GET_HIT(ch) -= number(1, 10)*2; alter_hit(ch,0);
  if (GET_HIT(ch) < 0)
  {
    GET_HIT(ch) = 0; alter_hit(ch,0);
  }
  ...
  if (!IS_SET(exitp->exit_info, EX_CLOSED)) {
    ...
    if (!check_falling(ch))
    {
      ...
      else
      {
        ...
        GET_MOVE(ch) -= 10; alter_move(ch,0);
  ...
  if (ch->skills)
  {
    if (ch->skills[SKILL_DOORBASH].learned)
    {
      ...
      else
      {
        ...
        GET_HIT(ch) -= number(1,5); alter_hit(ch,0);
...
if (GET_MANA(ch) < 15)  {
 ...
 if (ch->skills && ch->skills[SKILL_BELLOW].learned &&
   ...
   GET_MANA(ch)-=15; alter_mana(ch,0);
...
void do_doorway( struct char_data *ch, char *argument, int cmd)
{
  ...
  else if (dice(1,101) > ch->skills[SKILL_DOORWAY].learned)
  {
    ...
    GET_MANA(ch)-=10; alter_mana(ch,0);
  ...
  {
    GET_MANA(ch)-=20; alter_mana(ch,0);
...
void do_psi_portal( struct char_data *ch, char *argument, int cmd)
{
 ...
 /* Added for Pkillers Gaia 2001 */
  ...
  else if (dice(1,101) > ch->skills[SKILL_PORTAL].learned)      
  {
    ...
    GET_MANA(ch)-=37; alter_mana(ch,0);
  ...
  {
    GET_MANA(ch)-=75; alter_mana(ch,0);
    send_to_char("You close your eyes and open a portal and quickly step through.\n\r",ch);
...
void do_mindsummon( struct char_data *ch, char *argument, int cmd)
{
  ...
  if( !rp || rp->sector_type == SECT_AIR
   ...
   GET_MANA(ch) -=30;  alter_mana(ch,0);
...
void do_flame_shroud( struct char_data *ch, char *argument, int cmd)
{
  ...
  GET_MANA(ch) -= 40; alter_mana(ch,0);
...
void do_great_sight( struct char_data *ch, char *argument, int cmd)
{
  ...
  if (!(ch->skills[SKILL_GREAT_SIGHT].learned))   
  {
   ...
   if (ch->skills[SKILL_GREAT_SIGHT].learned < dice (1,101) )   {
     ...
     GET_MANA(ch) -= 25; alter_mana(ch,0);
     LearnFromMistake(ch, SKILL_GREAT_SIGHT, 0, 95);
     WAIT_STATE(ch, PULSE_VIOLENCE*3);
     return;
   }
 
  GET_MANA(ch) -= 50; alter_mana(ch,0);
...
void do_blast( struct char_data *ch, char *argument, int cmd)
{
  ...
  {
    GET_MANA(ch) -= 12; alter_mana(ch,0);
...
void do_blessing(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (number(1,101)>ch->skills[SKILL_BLESSING].learned)     
  {
    ...
    GET_MANA(ch) -= GET_LEVEL(ch,PALADIN_LEVEL_IND); alter_mana(ch,0);
  ...
  GET_MANA(ch) -= GET_LEVEL(ch,PALADIN_LEVEL_IND)*2; alter_mana(ch,0);
  factor=0;
  if (ch==dude)
    factor++;
...
if (IS_PC(ch) || IS_SET(ch->specials.act,ACT_POLYSELF))
   ...
   if (healing>wounds)
     ...
     GET_HIT(victim) +=healing; alter_hit(victim,0);
      ...
      GET_POS(ch) > POSITION_SITTING)
  {
    ...
    { /* made it */                                                  
     ...
     while( (obj) && (i<nling) ) 
     {
      ...
      {
        case 1:
          numdice = 1; sizedice = 1; break;
        case 2:
          numdice = 1; sizedice = 2; break;
        case 3:
          percent = number(1,2);
          if (percent == 1) { numdice = 1; sizedice = 3; }
          else { numdice = 2; sizedice = 1; }
          break;
        case 4:
          numdice = 1; sizedice = 4; break;
        case 5:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 5; }
          else if (percent == 2){ numdice = 2; sizedice = 2; }
          else { numdice = 3; sizedice = 1; }
          break;
        case 6:
          numdice = 1; sizedice = 6; break;
        case 7:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 7; }
          else if (percent == 2){ numdice = 2; sizedice = 3; }
          else { numdice = 4; sizedice = 1; }
          break;
        case 8:
          percent = number(1,2);
          if (percent == 1) { numdice = 1; sizedice = 8; }
          else { numdice = 3; sizedice = 2; }
          break;
        case 9:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 9; }
          else if (percent == 2){ numdice = 2; sizedice = 4; }
          else { numdice = 5; sizedice = 1; }
          break;
        case 10:
          numdice = 1; sizedice = 10; break;
        case 11:
          percent = number(1,5);
          if (percent == 1) { numdice = 1; sizedice = 11; }
          else if (percent == 2){ numdice = 2; sizedice = 5; }
          else if (percent == 3){ numdice = 3; sizedice = 3; }
          else if (percent == 4){ numdice = 4; sizedice = 2; }
          else { numdice = 6; sizedice = 1; }
          break;
        case 12:
          numdice = 1; sizedice = 12; break;
        case 13:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 13; }
          else if (percent == 2){ numdice = 2; sizedice = 6; }
          else { numdice = 7; sizedice = 1; }
          break;
        case 14:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 14; }
          else if (percent == 2){ numdice = 3; sizedice = 4; }
          else { numdice = 5; sizedice = 2; }
          break;
        case 15:
          percent = number(1,4);
          if (percent == 1) { numdice = 1; sizedice = 15; }
          else if (percent == 2){ numdice = 2; sizedice = 7; }
          else if (percent == 3){ numdice = 4; sizedice = 3; }
          else { numdice = 8; sizedice = 1; }
          break;
        case 16:
          numdice = 1; sizedice = 16; break;
        case 17:
          percent = number(1,4);
          if (percent == 1) { numdice = 1; sizedice = 17; }
          else if (percent == 2){ numdice = 2; sizedice = 8; }
          else if (percent == 3){ numdice = 3; sizedice = 5; }
          else { numdice = 9; sizedice = 1; }
          break;
        case 18:
          numdice = 1; sizedice = 18; break;
        case 19:
          percent = number(1,5);
          if (percent == 1) { numdice = 1; sizedice = 19; }
          else if (percent == 2){ numdice = 2; sizedice = 9; }
          else if (percent == 3){ numdice = 4; sizedice = 4; }
          else if (percent == 4){ numdice = 5; sizedice = 3; }
          else { numdice = 10; sizedice = 1; }
          break;
        case 20:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 20; }
          else if (percent == 2){ numdice = 3; sizedice = 6; }
          else { numdice = 7; sizedice = 2; }
          break;
        case 21:
          percent = number(1,2);
          if (percent == 1) { numdice = 1; sizedice = 21; }
          else { numdice = 2; sizedice = 10; }
          break;
        case 22:
          numdice = 1; sizedice = 22; break;
        case 23:
          percent = number(1,6);
          if (percent == 1) { numdice = 1; sizedice = 23; }
          else if (percent == 2){ numdice = 2; sizedice = 11; }
          else if (percent == 3){ numdice = 3; sizedice = 7; }
          else if (percent == 4){ numdice = 4; sizedice = 5; }
          else if (percent == 5){ numdice = 6; sizedice = 3; }
          else { numdice = 8; sizedice = 2; }
          break;
        case 24:
          percent = number(1,2);
          if (percent == 1) { numdice = 1; sizedice = 24; }
          else { numdice = 5; sizedice = 4; }
          break;
        case 25:
          percent = number(1,2);
          if (percent == 1) { numdice = 1; sizedice = 25; }
          else { numdice = 2; sizedice = 12; }
          break;
        case 26:
          percent = number(1,3);
          if (percent == 1) { numdice = 1; sizedice = 26; }
          else if (percent == 2){ numdice = 3; sizedice = 8; }
          else { numdice = 9; sizedice = 2; }
          break;
        case 27:
          percent = number(1,4);
          if (percent == 1) { numdice = 1; sizedice = 27; }
          else if (percent == 2){ numdice = 2; sizedice = 13; }
          else if (percent == 3){ numdice = 4; sizedice = 6; }
          else { numdice = 7; sizedice = 3; }
          break;
        case 28:
          numdice = 1; sizedice = 28; break;
        case 29:
        case 30:
          percent = number(1,6);
          if (percent == 1) { numdice = 1; sizedice = 29; }
          else if (percent == 2){ numdice = 2; sizedice = 14; }
          else if (percent == 3){ numdice = 3; sizedice = 9; }
          else if (percent == 4){ numdice = 5; sizedice = 5; }
          else if (percent == 5){ numdice = 6; sizedice = 4; }
          else { numdice = 10; sizedice = 2; }
          break;
        default:
