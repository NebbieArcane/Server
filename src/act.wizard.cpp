...
ACTION_FUNC(do_imptest) {
	...
	if (GetMaxLevel(ch) < MAESTRO_DEI_CREATORI) { return; }
...
void bamf(struct char_data* ch,const char* arg,int cmd) {
	...
	if (cmd==CMD_BAMFIN) {
		...
		if (ch->specials.poofin) {
			ch->specials.poofin=static_cast<char*>(realloc(ch->specials.poofin,len+1));
		}
		else {
		...
		if (ch->specials.poofout) {
			ch->specials.poofout=static_cast<char*>(realloc(ch->specials.poofout,len+1));
		}
		else {
	...
	if (arg) {
		...
		if ((tch = get_char_room_vis(ch,buf)) != 0) {
			if( GetMaxLevel(tch) >= MAESTRO_DEL_CREATO && IS_PC( tch ) &&
				...
				sprintf(mess, "Il tempo si ferma nello stupore mentre %s e %s si danno il 5!\n\r",
				...
				sprintf(mess, "Il Tempo si sposta in avanti e la realta' si distorce mentre %s e %s si danno il 5!\n\r",
						ch->player.name, tch->player.name);
				send_to_all(mess);
			}
			else if( GetMaxLevel(tch) >= DIO_MINORE && IS_PC( tch ) &&
					 GetMaxLevel(ch)  >= DIO_MINORE && IS_PC( ch ) ) {
				sprintf(mess, "Il tempo si ferma per un istante mentre %s e %s si danno il 5!\n\r",
						ch->player.name, tch->player.name);
				send_to_all(mess);
			}
			else {
...
// SALVO corretto crash su stat room con oggetto senza nome
		...
		/* mobile in world  PERSON! */
			...
			if (cmd ==CMD_NAME || cmd == CMD_CKEQ) {
				...
				sprintf(buf,"$c0005Level [M:$c0014%d$c0005/C:$c0015%d$c0005/W:$c0014%d$c0005/T:"
			...
			if (cmd == CMD_NAME) {
				sprintf(buf,"$c0005Xp:$c0014%d (%d)$c0007",GET_EXP(k),MIN_EXP(k));
				act(buf,FALSE,ch,0,0,TO_CHAR);
				...
				sprintf(buf, "$c0005Age: [$c0014%d (%d) $c0005] Y, [$c0014%d$c0005] M, "
			...
			if (cmd==CMD_STAT || cmd == CMD_CKEQ) { /* stat */
				...
				if (k==NULL || GET_NAME(k)==NULL) { return; } // SALVO la vittima non e' piu' tra noi dopo il controllo sugli hp
				act(buf,FALSE,ch,0,0,TO_CHAR);
			}
			if (cmd == CMD_STAT) {
				...
				sprintf(buf, "$c0005Supported is '$c0014%s$c0005'    ",
						(k->specials.supporting?k->specials.supporting:"NOBODY"));
				act(buf,FALSE,ch,0,0,TO_CHAR);
				sprintf(buf, "$c0005Bodyguarded is '$c0014%s$c0005'    ",
						(k->specials.bodyguarding?k->specials.bodyguarding:"NOBODY"));
				act(buf,FALSE,ch,0,0,TO_CHAR);
				sprintf(buf, "$c0005Bodyguard is '$c0014%s$c0005'    ",
						(k->specials.bodyguard?k->specials.bodyguard:"NOBODY"));
				act(buf,FALSE,ch,0,0,TO_CHAR);
			...
			if (cmd==CMD_CKEQ || cmd==CMD_STAT) { /* ckeq */
				sprintf(buf,"$c0005Equipment index:$c0014%f",GetCharBonusIndex(k));
				act(buf,FALSE,ch,0,0,TO_CHAR);
				sprintf(buf,"$c0005 (Medium value):$c0014%f",AverageEqIndex(-1));
				act(buf,FALSE,ch,0,0,TO_CHAR);
			}
	...
	if( !strcmp( field, "align" ) ) {
		...
		*/
		...
		{ mob->player.iClass = parm; }
			...
			sprintf(buf,"ATTENZIONE. USARE VALORI ESADECIMALI\nClass:%04Xh\n\r",mob->player.iClass);
			send_to_char(buf,ch);
		}
	}
...
void force_return(struct char_data* ch, const char* arg, int cmd) {
	...
	if(!ch->desc->original) {
		mudlog( LOG_CHECK,"%s has lost the original descriptor!!",ch->player.name );
		return;
	}
	else {
...
ACTION_FUNC(do_oload) {
	...
	if( obj_index[number].iVNum >= 150 && obj_index[number].iVNum < 200 &&
			! isname( "Alar",GET_NAME( ch )) && ! isname( "Salvo",GET_NAME( ch ))
	  ) { /*GGPATCH*/
...
void roll_abilities(struct char_data* ch) {
	...
	if (Rollata=='V'||Rollata=='S') {
		if (Rollata=='S') {
			...
			{ sprintf(newstats,"osdiwh"); }
			mudlog(LOG_CHECK,"%s ha rollato con metodo simple : %s",GET_NAME(ch),newstats);
		}
		else {
		...
		ch->abilities.intel = ch->abilities.intel-(18-MaxIntForRace(ch));
		...
		mudlog(LOG_PLAYERS,"%s ha rollato con metodo: %c",
			   GET_NAME(ch),Rollata);
...
ACTION_FUNC(do_advance) {
	...
	if (GET_LEVEL(victim, lin_class) == 0) {
		...
		if (GET_LEVEL(victim, lin_class) < MAESTRO_DEI_CREATORI) {
			long lGain = MAX( 0, titles[ lin_class ]
							  [ GET_LEVEL( victim, lin_class ) + adv ].exp
							  - GET_EXP( victim ) );
			gain_exp_regardless( victim, lGain, lin_class, MAESTRO_DEI_CREATORI );
...
ACTION_FUNC(do_immort) {
	...
	{ send_to_char("No-one by that name in the world.\n\r",ch); }
		...
		if (GetMaxLevel(victim)>=BARONE) {
			...
			act("$c0008L'oscurita' ti avvolge, senti la realta' torcersi e urlare \n\r"
				"Tutta la tua vita ti scorre davanti agli occhi....\n\r",
				FALSE, victim, 0, ch, TO_CHAR);
			act("Scorgi per un attimo un sorriso.. un sorriso di lupo.\n\n\r"
...
ACTION_FUNC(do_restore) {
	...
	{ send_to_char("No-one by that name in the world.\n\r",ch); }
		...
		if( IS_PC( ch ) && CAN_SEE(victim,ch) ) {
			if (GetMaxLevel(victim)<IMMORTALE)
				act("La mano di $N ti sfiora appena.... le tue ferite si rimarginano,\n\r"
					"Una nuova forza scorre in te!", FALSE, victim, 0, ch, TO_CHAR);
			else {
	...
	if (is_abbrev(buf, "zones")) {
		...
		for( zone = 0; zone<=top_of_zone_table; zone++ ) {
			...
			sprintf( buf,"%3d %6d - %-30.30s %4dm %4dm %6d-%-6d %s\n\r",
					 zone, zd->num, zd->name,
					 zd->lifespan, zd->age, zd->bottom, zd->top, mode );
			append_to_string_block(&sb, buf);
		}
	}
...
#else
			room_iterate(room_db, reinterpret_cast<iterate_func>(print_death_room), &sb);
#endif
...
#else
			room_iterate(room_db, reinterpret_cast<iterate_func>(print_private_room), &sb);
#endif
...
#else
			room_iterate(room_db, reinterpret_cast<iterate_func>(show_room_zone), &srzs);
#endif
...
ACTION_FUNC(do_invis) {
	...
	if (cmd == 242 && !IS_DIO_MINORE(ch)) { return; }
...
ACTION_FUNC(do_cset) {
	...
	if(!strcmp(buf1, "show")) {
		...
		if (n->min_level > GetMaxLevel(ch)) {
			send_to_char("Non hai il potere per modificare il livello di questo comando.\n\r", ch);
			return;
		}
...
ACTION_FUNC(do_god_interven) {
	...
	if (!strcmp("eclipse",tmp)) {
		if (IS_SET(SystemFlags,SYS_ECLIPS)) {
			...
			sprintf(buf, "The planets return to their normal orbit, slowly the "
		...
		if (IS_SET(SystemFlags,SYS_NOPORTAL)) {
			...
			send_to_char( "You scramble the planes to make portaling impossible.\n",
						  ch);
			sprintf(buf, "%s scrambles the planes to make portaling impossible.\n",
					GET_NAME(ch));
			send_to_all(buf);
		...
		if (IS_SET(SystemFlags,SYS_NOASTRAL)) {
			...
			send_to_char( "You shift the astral planes and make astral travel "
						  "impossible.\n",ch);
			sprintf(buf, "%s shifts the astral planes and make astral travel "
					"impossible.\n",GET_NAME(ch));
			send_to_all(buf);
		...
		if (IS_SET(SystemFlags,SYS_NOSUMMON)) {
			...
			send_to_char( "A magical fog spreads throughout the land making "
						  "summons impossible.\n", ch);
			sprintf(buf, "%s spreads a magical fog throughout the land making "
					"summons impossible.\n", GET_NAME(ch));
			send_to_all(buf);
		...
		if (IS_SET(SystemFlags,SYS_NOKILL)) {
			...
			send_to_char( "You spread thoughts of peace throught the people of "
						  "the land.\n",ch);
			sprintf(buf,"%s spreads thoughts of peace throught the people of "
					"the land.\n",GET_NAME(ch));
			send_to_all(buf);
...
ACTION_FUNC(do_nuke) {
	...
	if( GetMaxLevel( victim ) >= GetMaxLevel( ch ) ) {
		...
		system( buf );
		mudlog(LOG_PLAYERS,buf);
		sprintf( buf, "rm -f %s/%s.dead", PLAYERS_DIR, lower( GET_NAME( victim ) ) );
		system( buf );
		mudlog(LOG_PLAYERS,buf);
		sprintf( buf, "rm -f %s/%s.deaths", PLAYERS_DIR, lower( GET_NAME( victim ) ) );
		system( buf );
		mudlog(LOG_PLAYERS,buf);
...
ACTION_FUNC(do_wreset) { // SALVO aggiunto comando wreset
	...
	for (i = 0; i <= top_of_zone_table; i++) {
		if( zone_table[i].start == 0 ) {
			...
			mudlog( LOG_CHECK, "Performing GOD boot-time init of %d:%s (rooms %d-%d).",
	...
	sprintf(buf,"wreset ha inizializzato %d zone, %d gia' init, su %d.",c,z,top_of_zone_table+1);
	if (ch != NULL)
	{ send_to_char (buf,ch); }
	else
