...
DamageResult MissVictim( struct char_data *ch, struct char_data *v, int type, int w_type, DamageResult (*dam_func) (struct char_data *, struct char_data *, int, int, int ), int location);
int GetWeaponDam(struct char_data *ch, struct char_data *v, struct bj_data *wielded, int location);
int LoreBackstabBonus(struct char_data *ch, struct char_data *v);
DamageResult HitVictim( struct char_data *ch, struct char_data *v, int dam, int type, int w_type, DamageResult (*dam_func)( struct char_data *, struct char_data *, int, int, int ), int location);
DamageResult root_hit( struct char_data *ch, struct char_data *victim, int type, DamageResult (*dam_func)( struct char_data *, struct char_data *,int, int, int ), int DistanceWeapon, int location); 
...
switch( GET_RACE( ch ) )
{
  ...
  case RACE_DEMON: // Flyp 20180126 --> Race Demon is always evil!
