 ...
 */
 ...
 inline void logDefineStatusImpl(const char* name, bool value) { (void)name; (void)value; }
 // C++17: uso di struct anonima per avere una dichiarazione valida a livello namespace
 #define logDefineStatus(macroname) static struct _flag_status_##macroname { _flag_status_##macroname() { logDefineStatusImpl(#macroname, macroname); } } _flag_status_instance_##macroname
 #endif
 #ifndef ACCESSI
 #define ACCESSI				true
 #else
