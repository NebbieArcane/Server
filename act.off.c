...
/* Bene , stiamo combattendo.. le cose sono un po' piu' incasinate! */   
   if (IS_PC(ch->specials.fighting)) bonus=20;
   for( i = 0; i < 3; i++ )
   {
      attempt = number(0, 5);  /* Select a random direction */
      if( CAN_GO(ch, attempt) &&
...
void do_bodyguard(struct char_data *ch, char *argument, int cmd)
{
   ...
   if (victim->desc) ActionAlignMod(ch,victim,cmd);
...
if (IS_PC(ch) || IS_SET(ch->specials.act,ACT_POLYSELF))
 ...
 if (IS_SET(ch->specials.affected_by2,AFF2_BERSERK)) {
  ...
  if (IS_PC(ch) && (skillcheck > ch->skills[SKILL_BERSERK].learned))  {
    ...
    if (GET_POS(victim) > POSITION_DEAD)     {
     GET_MANA(ch) -=15; alter_mana(ch,0);  /* cost 15 mana to do it.. */
