...
class alarTemplateNode
{
	...
	function context($param)
	{
		...
		switch (strtoupper($param)) {
			...
			case "LAST":
				return $this->iteration == $this->total;
			case "INNER":
				return $this->iteration > 1 and $this->iteration < $this->total;
			case "COUNTER":
				return $this->iteration;
			case "EVEN":
				return ($this->iteration % 2);
			case "ODD":
				return (($this->iteration + 1) % 2);
			case "ROW{$this->iteration}":
				return true;
		}
		return false;
	}
