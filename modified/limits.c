...
int GetHpGain(struct char_data *ch, int iClass,int livello,int compat) 
{
   int add_hp;
...
int gain_corretto(struct char_data *ch,int gain) 
{
   ...
   if (abs((int)eqratio)<100) return(gain);
   tmp=eqratio/20;
   if(tmp < -30) tmp = -30 ;
   if(tmp > 30) tmp = 30 ;
   tmp=gain*tmp/100;
			...
			titles[ i ][ GET_LEVEL( ch, i ) + 2 ].exp) 
		     {
		     	...
		     	if ((titles[ i ][ GET_LEVEL( ch, i ) + 2 ].exp - 1)>0)
		     		...
		     		send_to_char("Non pigli exp perchè passi la soglia massima!!!",ch);
			return;
		     }
		  }
	       }
			 ...
			 int iMaxLevel )
{
   ...
   if( !IS_NPC( ch ) )
   {
      ...
      else if( gain < 0 ) GET_EXP( ch ) += gain;
...
{
   if (IS_LINKDEAD(ch) && ch->specials.timer < 30) ch->specials.timer=30;
...
void ClassSpecificStuff( struct char_data *ch)
{
       ...
       HasClass(ch,CLASS_RANGER))   
   {
	 ...
	 /* fix up damage stuff */
         addlev = GET_LEVEL(ch, MONK_LEVEL_IND) ; // Gaia 2001
         if(IS_SINGLE(ch)) addlev++ ;
         mudlog( LOG_CHECK, "%s livello del monaco: %d", GET_NAME( ch ), addlev ) ;
	 switch(addlev) 
	 {
	  case 1:
	  case 2:
   ...
   mudlog(LOG_PLAYERS,"%s: numatcks = %f",GET_NAME(ch),ch->mult_att);
