...
uint16_t NumTimeCheck =
    0; /* indice anello per aTimeCheck[] (modulo PULSE_MOBILE) */
...
int run(int port, const char *dir) {
  ...
  mudlog(LOG_ALWAYS, "Pulse:%-10s%-10s%-10s%-10s%-10s%-10s", "zone", "river",
         "teleport", "violence", "mobile", "tick");
  mudlog(LOG_ALWAYS, "Value:%10d%10d%10d%10d%10d%10d", PULSE_ZONE, PULSE_RIVER,
         PULSE_TELEPORT, PULSE_VIOLENCE, PULSE_MOBILE,
         (PULSE_PER_SEC * SECS_PER_MUD_HOUR));
...
void game_loop(int s) {
  ...
  std::chrono::time_point<steady_clock, steady_clock::duration> next_tick =
      steady_clock::now();
  /* Main loop */
  while (!mudshutdown) {
    ...
    /**
     * Changed timing out selct with thread_sleep as a first step to a
     * multithuread version We could at least move the non playing sockets to
     * their own thread
     */
    do {
    ...
    for (point = descriptor_list; point; point = next_to_process) {
      ...
      if (point->connected == CON_PLYNG) {
        ...
        if ((static_cast<long long>(point->wait) <
          ...
          if (point->connected == CON_EDITING ||
              point->connected == CON_OBJ_EDITING ||
              point->connected == CON_MOB_EDITING) {
            write_to_descriptor(point->descriptor,
                                "Smetti di plasmare la materia.\n\r");
            act("$n smette di "
      ...
      const long long wait_after = static_cast<long long>(point->wait) - 1LL;
  ...
  /**
   * Changed timing out select with thread_sleep as a first step to a
   * multithuread version We could at least move the non playing sockets to
   * their own thread Also moved to the end of the loop
   */
  microseconds lag(1);
  do {
    std::this_thread::sleep_until(next_tick);
...
int new_descriptor(int s) {
  ...
  /* Limite connessioni: conta le sessioni MUD, non il numero del file
   * descriptor. */
  ...
  if (getpeername(desc, (struct sockaddr *)&sock, &size) < 0) {
    ...
    *newd->host = '\0';
    ...
    sprintf(newd->host, "%d.%d.%d.%d", (i & 0xFF000000) >> 24,
            (i & 0x00FF0000) >> 16, (i & 0x0000FF00) >> 8, (i & 0x000000FF));
  } else {
  ...
  *newd->last_input = '\0';
  mudlog(LOG_CONNECT, "New connection from addr [HOST:%s]: %d: %d (wait: %d)",
         newd->host, desc, maxdesc, newd->wait);
#if not BLOCK_WRITE
  ...
  *(newd->output) = '\0';
  ...
  SEND_TO_Q(ParseAnsiColors(
                TRUE, "$c0007"
                      "$c0011Inserisci l'$c0012email del tuo account su "
                      "http://www.nebbiearcane.it$c0011 o il $c0004nome$c0011 "
                      "di un personaggio.\r\n"
                      "Se non hai o non ricordi il codice di accesso (diverso "
                      "dalla password del sito) vai su "
...
int write_to_descriptor(int desc, char *txt) {
  ...
  for (iStart = 0; iStart < length; iStart += nWrite) {
    ...
    if ((nWrite = write(desc, txt + iStart, nBlock)) < 0) {
      ...
      mudlog(LOG_ERROR,
             "<#=%d> had a error (%d) in write to descriptor "
             "(Broken Pipe?)",
             desc, errno);
...
void nonblock(int s) {
  ...
  if (fcntl(s, F_SETFL, nFlags) < 0) {
    mudlog(LOG_ERROR, "%s:%s",
           "Fatal error executing nonblock (comm.c,strerror(errno))");
  }
  return;
}
...
void send_to_all(const char *messg) {
  ...
  if (messg)
    for (i = descriptor_list; i; i = i->next)
      if (!i->connected)
        SEND_TO_Q(ParseAnsiColors(
                      IS_SET(i->character->player.user_flags, USE_ANSI), messg),
                  i);
}
  ...
  if (!IS_SET(real_roomp(room)->room_flags, SILENCE)) {
    for (i = real_roomp(room)->listeners; i; i = i->next_listener) {
      if (i->desc) {
        ...
        SEND_TO_Q(ParseAnsiColors(IS_SET(i->player.user_flags, USE_ANSI), buf),
                  i->desc);
      }
    }
  }
...
void ParseAct(const char *str, struct char_data *ch, struct char_data *to,
              void *vict_obj, struct obj_data *obj, char *buf)
...
void act(const char *str, int hide_invisible, struct char_data *ch,
  ...
  for (; to; to = to->next_in_room) {
    if (to->desc && (!hide_invisible || CAN_SEE(to, ch)) &&
...
void construct_prompt(char *outbuf, struct char_data *ch) {
  ...
  *outbuf = 0;
  ...
  if (!ch->specials.fighting && ch->specials.supporting &&
...
#define DUMMYVALUE 999999999
          for (l = 1, i = 0, texp = exp = DUMMYVALUE; i <= PSI_LEVEL_IND;
               i++, l <<= 1) {
            if (HasClass(ch, l)) {
        ...
        case 'c': /* tank condition */
          if (ch->specials.fighting &&
            ...
            if (i >= 100) {
              ...
              if (GET_SEX(ch->specials.fighting->specials.fighting) ==
                  SEX_FEMALE)
                strcpy(tbuf, "$c0002graffiata$c0007");
              else
                strcpy(tbuf, "$c0002graffiato$c0007");
...
int update_max_usage(void) {
  ...
  if (sockets_playing > max_usage) {
    ...
    if (PORT == RELEASE_PORT) {
      ...
      slackNotify(buf,
                  max_usage >= 10 ? ":champagne:" : "information_desk_person");
    }
